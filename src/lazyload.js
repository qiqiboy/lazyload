...
(function(ROOT, NS, Struct, undefined) {
    ...
    var DOC = ROOT.document,
                ...
                return (
                    ...
                    (elem.lazyData = {
                        ...
                        tick: function() {
                            ...
                            data.WST =
                                (isWin
                                    ? win.pageYOffset || (doc && doc.scrollTop) || body.scrollTop
                                    : getOffset(win).top) || 0;
                            data.WSL =
                                (isWin
                                    ? win.pageXOffset || (doc && doc.scrollLeft) || body.scrollLeft
                                    : getOffset(win).left) || 0;
                            data.WH =
                                (isWin
                                    ? win.innerHeight || (doc && doc.clientHeight) || body.clientHeight
                                    : win.clientHeight) || 0;
                            data.WW =
                                (isWin
                                    ? win.innerWidth || (doc && doc.clientWidth) || body.clientWidth
                                    : win.clientWidth) || 0;
        ...
        isHidden = function(elem) {
            ...
            return (
                (elem.currentStyle || getComputedStyle(elem, null) || elem.style).display == 'none' ||
                (doc !== elem &&
                    !(doc.contains
                        ? doc.contains(elem)
                        : doc.compareDocumentPosition && doc.compareDocumentPosition(elem) & 16))
            );
        },
        type = function(obj) {
            if (obj == null) {
    ...
    Struct.fn = Struct.prototype = {
        ...
        init: function(elem, range, callback) {
            ...
            if (tp == 'function') {
                ...
                if (
                    ...
                    container.nodeName.toLowerCase() == 'html'
                ) {
                    container = ROOT;
                }
            }
        ...
        isArrayLike: function(elem) {
            ...
            return (
                ...
                tp != 'function' &&
                tp != 'string' &&
                (elem.length === 0 || (elem.length && elem.length - 1 in elem))
            );
        },
        merge: function(elem) {
            var i = this.length,
        ...
        check: function() {
            ...
            while (i < this.length) {
                ...
                if (
                    !isHidden(elem) &&
                    offset.top + elem.offsetHeight + hrange >= data.WST &&
                    offset.top - hrange <= data.WST + data.WH &&
                    offset.left + elem.offsetWidth + wrange >= data.WSL &&
                    offset.left - wrange <= data.WSL + data.WW
                ) {
